...
reverseList2 :: [a] -> [a]
reverseList2 [] = []
reverseList2 (x : xl) = reverseList2 xl ++ [x] ---pattern matching mai mult ca am (x:xs)
...
numToBase n b = if n == 0 then [] else numToBase (n `div` b) b ++ [mod n b]

-- pattern matching
numToBase2 :: Integer -> Integer -> [Integer]
numToBase2 0 _ = []
numToBase2 n b = numToBase2 (n `div` b) b ++ [mod n b]

-- gărzi
numToBase3 :: Integer -> Integer -> [Integer]
numToBase3 n b
    | n == 0 = []
    | otherwise = numToBase3 (n `div` b) b ++ [mod n b]

-- case of
numToBase4 :: Integer -> Integer -> [Integer]
numToBase4 n b = case n of
    0 -> []
    _->numToBase4 (n `div` b) b ++ [mod n b]
...
mergeSort :: [Int] -> [Int]
mergeSort lst = let computeHalf f l = f  (quot (length l) 2) l  ---drop / take == f de fapt si 1 arg e cate elem iau din l
...
mergeSort2 lst = case lst of
                    ...
                    _-> merge (mergeSort (fstHalf lst)) (mergeSort (sndHalf lst))
                    where
                        computeHalf f l = f  (quot (length l) 2) l  ---drop / take == f de fapt si 1 arg e cate elem iau din l
                        fstHalf l = computeHalf take l
                        sndHalf l = computeHalf drop l
