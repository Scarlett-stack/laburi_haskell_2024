...
---caz nul
insertElem root@(BSTNode val lt rt) elem
        |val == elem = root 
        |val < elem = BSTNode val lt (insertElem rt elem) --continuam!
        |val > elem = BSTNode val (insertElem lt elem) rt

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing --gen NULL
findElem (BSTNode vl lt rt) elem
      | vl == elem = Just vl --pe aia o returnez!
      | vl < elem = findElem rt elem
      | vl > elem = findElem lt elem

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode vl lt rt) = inorder lt ++ [vl] ++ inorder rt 

size :: BST a -> Int
size BSTNil = 0
size (BSTNode vl lt rt) = 1 + size lt + size rt

height :: BST a -> Int
height BSTNil = 0
height (BSTNode vl lt rt) = 1 + max (height rt) (height lt) --are sens ca nu poti numara in ambele parti trb doar una
...
emptyList = List [] --construesc tipul List cu val []


consElem :: a -> NestedList a -> NestedList a
consElem x (Atom y) = List [Atom x,  Atom y] --am doar 2 elemente doar modific campurile atom
consElem x (List xs) = List (Atom x : xs) --fac u  atom din restul listei si x?
