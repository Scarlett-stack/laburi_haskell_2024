...
class Container t where ---interfete
    contents :: t a -> [a]
...
class Invertible a where ---interfete
    invert :: a -> a
...
-}
--- trb functie aditionala care face moftul ala
customPrint :: Show a => Char -> Int -> BST a -> [Char]
customPrint _ _ BSTNil = ""
customPrint tabChar lvl (BSTNod val l r) = replicate lvl tabChar ++ show val ++ "\n" 
    ++ customPrint tabChar (lvl + 1 ) l
    ++ customPrint tabChar (lvl + 1) r
instance Show a => Show (BST a) where
    show BSTNil = ""
    show tree = customPrint '\t' 0 tree
...
instance Invertible (BST a) where
    ...
    invert (BSTNod v1 l1 r1) = BSTNod v1 (invert r1) (invert l1)
...
instance Functor BST where
    ...
    fmap f (BSTNod a l r) = BSTNod (f a) (fmap f l) (fmap f r)
...
instance Foldable BST where
    ...
    foldr f acc (BSTNod v l r) = foldr f (f v newAcc) l
        where 
            newAcc = foldr f acc r
